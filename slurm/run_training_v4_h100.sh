#!/bin/bash
# v4 H100: same as v3 H100 (Pi0 sparse-upcycled MoE + diversity) plus the
# layer-conditioned discriminator and per-layer orth loss
# (see src/lerobot/policies/smolvla/moe.py). Distinct OUTDIR/seed/job-name so
# it can run alongside any earlier v3 jobs without clobbering anything.
set -uo pipefail

REPO=/scratch/gpfs/FHEIDE/rj2807/vla-moe-diversity
LOGDIR=/scratch/gpfs/FHEIDE/rj2807/logs
OUTDIR=/scratch/gpfs/FHEIDE/rj2807/outputs/pi0_moe_upcycled_v4_h100
mkdir -p "$LOGDIR"
...
python -m lerobot.scripts.lerobot_train \
  ...
  --seed=1002 \
  --output_dir="$OUTDIR" \
  --job_name=pi0_moe_upcycled_v4_h100 \
  --wandb.enable=true \
  --wandb.mode=offline \
