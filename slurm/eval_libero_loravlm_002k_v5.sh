#!/bin/bash
# v5-routing eval of Job B (libero+LoRA-VLM bottleneck) at step-2K.
set -eo pipefail

REPO=/scratch/gpfs/EYSENBACH/ss5822/vla-moe-diversity
CKPT=${REPO}/outputs/pi0_bottleneck5_libero_loravlm/checkpoints/002000/pretrained_model
OUT=${REPO}/outputs/eval/libero_loravlm_step002k_v5
LOG_DIR=${REPO}/logs
mkdir -p "${LOG_DIR}"
LOG="${LOG_DIR}/eval_libero_loravlm_002k_v5_$(date +%Y%m%d_%H%M%S).log"
...
exec python -m lerobot.scripts.eval_v5_routing \
  ...
  --eval.n_episodes=1 \
  --output_dir="${OUT}" \
  --job_name=eval_libero_loravlm_002k_v5 \
  --seed=1000 \
