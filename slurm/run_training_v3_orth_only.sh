#!/bin/bash
# v3 ablation: upcycled MoE with ONLY the orthogonality loss (no discriminability).
# Tests Kim 2026's claim that the orthogonality loss alone is ineffective.
set -uo pipefail

REPO=/scratch/gpfs/FHEIDE/rj2807/vla-moe-diversity
LOGDIR=/scratch/gpfs/FHEIDE/rj2807/logs
OUTDIR=/scratch/gpfs/FHEIDE/rj2807/outputs/pi0_moe_upcycled_v3_orth_only
mkdir -p "$LOGDIR"
...
python -m lerobot.scripts.lerobot_train \
  ...
  --seed=1003 \
  --output_dir="$OUTDIR" \
  --job_name=pi0_moe_upcycled_v3_orth_only \
  --wandb.enable=true \
  --wandb.mode=offline \
