#!/bin/bash
# v6 disc-only ablation (no-orth): orth=0, disc=0.02.
# Tests whether the discriminability term alone produces diverse experts.
# Runs on della-stellato GPU 1 (H100 NVL 95GB) outside slurm, in a tmux session.
set -uo pipefail

REPO=/scratch/gpfs/FHEIDE/rj2807/vla-moe-diversity
LOGDIR=/scratch/gpfs/FHEIDE/rj2807/logs
OUTDIR=/scratch/gpfs/FHEIDE/rj2807/outputs/pi0_moe_v6_no_orth_h100
mkdir -p "$LOGDIR"
...
python -m lerobot.scripts.lerobot_train \
  ...
  --seed=1007 \
  --output_dir="$OUTDIR" \
  --job_name=pi0_moe_v6_no_orth_h100 \
  --wandb.enable=true \
  --wandb.mode=offline \
