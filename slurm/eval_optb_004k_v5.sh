#!/bin/bash
# v5-routing eval of Job A (option b) at step-4K checkpoint, 1 rollout per
# LIBERO-10 task. Tests whether bottlenecking state too breaks the policy
# entirely or preserves some success.
set -eo pipefail

REPO=/scratch/gpfs/EYSENBACH/ss5822/vla-moe-diversity
CKPT=${REPO}/outputs/pi0_bottleneck5_optb/checkpoints/004000/pretrained_model
OUT=${REPO}/outputs/eval/optb_step004k_v5
LOG_DIR=${REPO}/logs
mkdir -p "${LOG_DIR}"
LOG="${LOG_DIR}/eval_optb_004k_v5_$(date +%Y%m%d_%H%M%S).log"
