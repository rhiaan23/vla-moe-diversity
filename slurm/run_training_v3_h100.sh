#!/bin/bash
# v3 H100 variant: parallel run on della-fongpu H100 NVL alongside the A100 v3 run.
# Distinct OUTDIR/seed/job-name to avoid clobbering the A100 run that uses
# slurm/run_training_v3_local.sh.
set -uo pipefail

REPO=/scratch/gpfs/FHEIDE/rj2807/vla-moe-diversity
LOGDIR=/scratch/gpfs/FHEIDE/rj2807/logs
OUTDIR=/scratch/gpfs/FHEIDE/rj2807/outputs/pi0_moe_upcycled_v3_h100
mkdir -p "$LOGDIR"
...
python -m lerobot.scripts.lerobot_train \
  ...
  --seed=1001 \
  --output_dir="$OUTDIR" \
  --job_name=pi0_moe_upcycled_v3_h100 \
  --wandb.enable=true \
  --wandb.mode=offline \
