#!/bin/bash
# Compositional-generalization probe variant B: same as compgen_t0 but with
# the two subtask order swapped (tomato first, then alphabet).
set -eo pipefail
...
OUT=${REPO}/outputs/eval/bottleneck5_step010k_compgen_t0_b
LOG_DIR=${REPO}/logs
mkdir -p "${LOG_DIR}"
LOG="${LOG_DIR}/eval_bottleneck5_010k_compgen_t0_b_$(date +%Y%m%d_%H%M%S).log"
...
exec python -m lerobot.scripts.eval_v5_routing \
  ...
  --env.task=libero_10 \
  --env.task_ids='[0]' \
  --env.instruction_override="first put the tomato soup in the basket and then put the alphabet soup in the basket" \
  --eval.batch_size=5 \
  --eval.n_episodes=5 \
  --output_dir="${OUT}" \
  --job_name=eval_bottleneck5_010k_compgen_t0_b \
  --seed=1000 \
