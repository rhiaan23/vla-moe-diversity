#!/bin/bash
# Single-checkpoint eval helper for parallel-host re-evaluation.
# Usage: reeval_one.sh <TAG> <CKPT_DIR>
# Example: reeval_one.sh bottleneck16_optb /scratch/.../checkpoints/last/pretrained_model
set -eo pipefail
...
OUT=${REPO}/outputs/eval/${TAG}_correct
LOG_DIR=${REPO}/logs
mkdir -p "${LOG_DIR}"
LOG=${LOG_DIR}/eval_${TAG}_correct_$(date +%Y%m%d_%H%M%S).log
...
echo "[reeval_one $(date +%H:%M:%S)] tag=${TAG}  ckpt=${CKPT}  out=${OUT}  host=$(hostname)" | tee -a "${LOG}"
...
echo "[reeval_one $(date +%H:%M:%S)] DONE tag=${TAG}  host=$(hostname)  pc_success=${PC}" | tee -a "${LOG}"
