#!/bin/bash
# Pi0 v5 (whole-expert MoE + LoRA) + 16-D prefix bottleneck w/ option (b):
...
# all observation info (image+lang+state) compressed into the 16-D z control
# vector. Same as pi0_bottleneck5_optb.sh but with a wider bottleneck.
set -eo pipefail

REPO=/scratch/gpfs/EYSENBACH/ss5822/vla-moe-diversity
OUT=${REPO}/outputs/pi0_bottleneck16_optb
LOG_DIR=${REPO}/logs
mkdir -p "${LOG_DIR}"
LOG="${LOG_DIR}/pi0_bottleneck16_optb_$(date +%Y%m%d_%H%M%S).log"
...
exec python -m lerobot.scripts.lerobot_train \
  ...
  --log_freq=200 \
  --output_dir="${OUT}" \
  --job_name=pi0_bottleneck16_optb \
  --seed=1003 \
  --wandb.enable=true \
  --wandb.mode=offline \
  --wandb.disable_artifact=true \
  --wandb.project=vla-moe-diversity \
  --wandb.notes="pi0 v5 + 16-D bottleneck OPTION B (state in z, no state token in suffix), libero_10, 10k steps" \
