#!/bin/bash
# v5-routing eval of Job A (option b) at step-10K final checkpoint.
set -eo pipefail

REPO=/scratch/gpfs/EYSENBACH/ss5822/vla-moe-diversity
CKPT=${REPO}/outputs/pi0_bottleneck5_optb/checkpoints/last/pretrained_model
OUT=${REPO}/outputs/eval/optb_step010k_v5
LOG_DIR=${REPO}/logs
mkdir -p "${LOG_DIR}"
LOG="${LOG_DIR}/eval_optb_010k_v5_$(date +%Y%m%d_%H%M%S).log"
