#!/bin/bash
# v3 ablation: upcycled MoE with diversity losses OFF (standard MoE + sparse-upcycled init).
# Pairs with run_training_v3_h100.sh (full diversity) and run_training_v3_orth_only.sh (orth only)
# to form the 3-way ablation for the workshop paper.
set -uo pipefail

REPO=/scratch/gpfs/FHEIDE/rj2807/vla-moe-diversity
LOGDIR=/scratch/gpfs/FHEIDE/rj2807/logs
OUTDIR=/scratch/gpfs/FHEIDE/rj2807/outputs/pi0_moe_upcycled_v3_no_diversity
mkdir -p "$LOGDIR"
...
python -m lerobot.scripts.lerobot_train \
  ...
  --seed=1002 \
  --output_dir="$OUTDIR" \
  --job_name=pi0_moe_upcycled_v3_no_diversity \
  --wandb.enable=true \
  --wandb.mode=offline \
