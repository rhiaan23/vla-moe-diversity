#!/bin/bash
# v5-routing eval of Job B (libero+LoRA-VLM bottleneck) at step-6K.
# PEFT auto-detected from config.json (use_peft=True) + adapter_config.json.
set -eo pipefail

REPO=/scratch/gpfs/EYSENBACH/ss5822/vla-moe-diversity
CKPT=${REPO}/outputs/pi0_bottleneck5_libero_loravlm/checkpoints/last/pretrained_model
OUT=${REPO}/outputs/eval/libero_loravlm_step006k_v5
LOG_DIR=${REPO}/logs
mkdir -p "${LOG_DIR}"
LOG="${LOG_DIR}/eval_libero_loravlm_006k_v5_$(date +%Y%m%d_%H%M%S).log"
...
exec python -m lerobot.scripts.eval_v5_routing \
  ...
  --eval.n_episodes=1 \
  --output_dir="${OUT}" \
  --job_name=eval_libero_loravlm_006k_v5 \
  --seed=1000 \
