#!/bin/bash
# Compositional-generalization probe of the step-10K bottleneck5 checkpoint.
# Task 0 of LIBERO-10 only, but with the language instruction overridden to a
# two-stage pick-and-place prompt to see whether the policy can chain two
# subtasks from a single high-level command.
set -eo pipefail
...
OUT=${REPO}/outputs/eval/bottleneck5_step010k_compgen_t0
LOG_DIR=${REPO}/logs
mkdir -p "${LOG_DIR}"
LOG="${LOG_DIR}/eval_bottleneck5_010k_compgen_t0_$(date +%Y%m%d_%H%M%S).log"
...
exec python -m lerobot.scripts.eval_v5_routing \
  ...
  --env.task=libero_10 \
  --env.task_ids='[0]' \
  --env.instruction_override="first put the alphabet soup in the basket and then put the tomato soup in the basket" \
  --eval.batch_size=5 \
  --eval.n_episodes=5 \
  --output_dir="${OUT}" \
  --job_name=eval_bottleneck5_010k_compgen_t0 \
  --seed=1000 \
